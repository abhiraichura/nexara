...
import { Assets, Goals, Risk, PayrollRun, SupplierComparison, InvoiceGenerator, DebtTender, Training, RegulatoryQA, Insurance, Performance } from './modules2';
...
export default function App() {
  ...
  const saveTxns = v => { save('txns', v); setTxns(v); };
  const saveEmps = v => { save('emps', v); setEmps(v); };
  const saveInv = v => { save('inv', v); setInv(v); };
  const saveVendors = v => { save('vendors', v); setVendors(v); };
  const saveLeads = v => { save('leads', v); setLeads(v); };
  const saveDocs = v => { save('docs', v); setDocs(v); };
  const saveJobs = v => { save('jobs', v); setJobs(v); };
  const saveContracts = v => { save('contracts', v); setContracts(v); };

  useEffect(() => { if (co) save('co', co); }, [co]);
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', background: bg, fontFamily: f, overflow: 'hidden' }}>
      ...
      {/* AI Assistant */}
      <AIPanel {...{ co, txns, emps, inv, leads, sales, expenses, profit, gstNet, fmt, aiChat, setAiChat, aiLoading, setAiLoading }} />
    </div>
  );
}
...
function Accounting({ txns, saveTxns, fmt, fmtN, fmtD, today, uid, log, GST_RATES, TXN_CATS }) {
  ...
  const [form, setForm] = useState({ type: 'income', date: today(), party: '', note: '', cat: TXN_CATS[0], amount: '', gst: '18', hsn: '', status: 'cleared' });
  ...
  const openAdd = (t) => {
    if (t) {
      ...
      setForm({ type: 'income', date: today(), party: '', note: '', cat: TXN_CATS[0], amount: '', gst: '18', hsn: '', status: 'cleared' });
      setEditId(null);
    }
    setShowAdd(true);
  };
    ...
    try {
      const r = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: 'user', content: `Write a professional business proposal letter from ${co.name} (${co.city}, ${co.state}) to ${lead.co}${lead.contact ? ', Attn: ' + lead.contact : ''}. Deal value: ${lead.val ? fmtN(lead.val) : 'To be discussed'}. Stage: ${lead.stage}. Keep it formal, concise (3-4 paragraphs), and persuasive. Include a clear call to action.` }],
        }),
      });
      const d = await r.json();
      setDraft(d.content?.[0]?.text || 'Could not generate draft.');
    ...
    try {
      ...
      const r = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [{ role: 'user', content: `Based on the following documents:\n\n${context}\n\nAnswer this question: ${query}\n\nCite which document your answer comes from.` }],
        }),
      });
      const d = await r.json();
      setAnswer(d.content?.[0]?.text || 'No answer generated.');
  ...
  const generate = async (tpl) => {
    ...
    try {
      const r = await fetch('https://api.anthropic.com/v1/messages', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ model: 'claude-sonnet-4-6', max_tokens: 600, messages: [{ role: 'user', content: tpl.prompt() }] }),
      });
      const d = await r.json();
      setDrafts(p => ({ ...p, [tpl.id]: d.content?.[0]?.text || 'Generation failed.' }));
...
function Settings({ co, setCo, save, load }) {
  ...
  const restore = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        alert('Restore successful. Refresh the app to see restored data.');
  ...
  return (
    <div>
      ...
      <Tabs tabs={[{ id: 'firm', label: 'Firm Profile' }, { id: 'data', label: 'Data & Backup' }, { id: 'about', label: 'About Nexara' }]} active={tab} onChange={setTab} />
...
// ─── AI PANEL ──────────────────────────────────────────────────────────────────
function AIPanel({ co, txns, emps, inv, leads, sales, expenses, profit, gstNet, fmt, aiChat, setAiChat, aiLoading, setAiLoading }) {
  const [open, setOpen] = useState(false);
  const [msg, setMsg] = useState('');
  const endRef = useRef(null);

  useEffect(() => { endRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [aiChat]);

  const ask = async () => {
    if (!msg.trim() || aiLoading) return;
    const userMsg = msg; setMsg('');
    const newChat = [...aiChat, { role: 'user', content: userMsg }];
    setAiChat(newChat); setAiLoading(true);

    const context = `Company: ${co.name} (${co.industry}, ${co.city}). Revenue: ${fmt(sales)}. Expenses: ${fmt(expenses)}. Net Profit: ${fmt(profit)}. GST Payable: ${fmt(gstNet)}. Employees: ${emps.length}. Inventory items: ${inv.length}. Active leads: ${leads.filter(l=>!['Won','Lost'].includes(l.stage)).length}.`;
    try {
      const r = await fetch('https://api.anthropic.com/v1/messages', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...
          system: `You are a private business advisor for ${co.name}. Business context: ${context}. Give concise, practical advice based on their actual data. Answer in plain English.`,
          messages: newChat.map(m => ({ role: m.role, content: m.content })),
        }),
      });
      const d = await r.json();
      setAiChat([...newChat, { role: 'assistant', content: d.content?.[0]?.text || 'No response.' }]);
    } catch { setAiChat([...newChat, { role: 'assistant', content: 'Could not connect. Check your internet.' }]); }
    setAiLoading(false);
  };
    ...
    <>
      ...
      <button onClick={() => setOpen(o => !o)}
        style={{
          ...
          borderRadius: '50%', background: T.am, border: 'none', color: '#FFF',
          fontSize: 20, cursor: 'pointer', boxShadow: '0 4px 14px rgba(194,89,10,0.4)',
      ...
      {open && (
        <div style={{
          ...
          <div style={{ padding: '14px 16px', borderBottom: `1px solid ${T.bd}`, background: T.sb }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: '#FFF' }}>Nexara AI Advisor</div>
            <div style={{ fontSize: 10, color: '#6B5E4E', fontFamily: mo, marginTop: 2 }}>Context: {co.name} · {fmt(sales)} revenue</div>
          </div>
          <div style={{ flex: 1, overflowY: 'auto', padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
            {aiChat.length === 0 && (
              <div style={{ color: T.t4, fontSize: 12, textAlign: 'center', padding: '20px 0', fontFamily: mo }}>
                Ask anything about your business…
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 12 }}>
                  {['What is my current profit margin?', 'Am I going to have a cash flow problem?', 'Which are my best performing sales categories?'].map(q => (
                    <button key={q} onClick={() => setMsg(q)}
                      style={{ fontSize: 11, padding: '6px 10px', background: T.sf2, border: `1px solid ${T.bd}`, borderRadius: 5, cursor: 'pointer', color: T.t2, textAlign: 'left' }}>
                      {q}
                    </button>
                  ))}
                </div>
            ...
            {aiLoading && <div style={{ fontSize: 11, color: T.t4, fontFamily: mo }}>Thinking…</div>}
            <div ref={endRef} />
          </div>
          <div style={{ padding: '10px 12px', borderTop: `1px solid ${T.bd}`, display: 'flex', gap: 8 }}>
            <input value={msg} onChange={e => setMsg(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); ask(); } }}
              placeholder="Ask about your business…"
              style={{ flex: 1, padding: '7px 10px', background: T.sf2, border: `1px solid ${T.bd}`, borderRadius: 6, color: T.tx, fontSize: 12, fontFamily: f, outline: 'none' }} />
            <BtnP small onClick={ask} disabled={aiLoading || !msg.trim()}>→</BtnP>
          </div>
        </div>
      )}
    </>
